...
def fetch_hackernews_signals(*, vertical_id: int, limit: int = 25) -> List[Dict[str, Any]]:
    ...
    for d in ingest_hackernews(client=client, vertical_id=int(vertical_id), cfg=cfg):
        if d.get("content"):
            out.append(d)
    return out
